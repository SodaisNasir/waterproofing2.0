...
import { upload_url } from '../../constants/BaseURl';

const windowWidth = Dimensions.get('screen').width;
const windowHeight = Dimensions.get('screen').height;
const AddLead = ({ route }) => {
  const { client_details, reset } = route.params;
  ...
  return (
    <LinearGradient
      ...
      <AddLeadUI client_details={client_details} reset={reset} />
    </LinearGradient>
  );
};
const AddLeadUI = ({ client_details, reset }) => {
      ...
      <View style={styles.card}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
              ...
              return (
                <View key={index}>
                  <Image
                    key={index}
                    style={styles.box}
                    source={{ uri: upload_url + item.uri}}
                  />
                  <AntDesign
                    onPress={() => Delete(item)}
                    style={styles.icon}
          ...
          <CustomButton
            ...
            onPress={() => {
              navigation.navigate('addDetails2', {
                client_details,
                video: video,
                images: images,
                reset
